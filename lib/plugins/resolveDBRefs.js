 ...
 * @api public
 */
var resolveDBRefs = function resolveDBRefs(schema, options) {
    if (!('mongoose' in options)) {
        throw new Error('\'mongoose\' option not defined');
    }

    if (!('DBRef' in options.mongoose.Schema.Types)) {
        throw new Error('\'DBRef\' type not installed');
    }

    var dbref = options.mongoose.SchemaType.DBRef;
    schema.eachPath(
        function(path, defn) {
                ...
                if (defn.options.cached === true) {
                    ...
                    schema.method(names.getter, function(callback) {
                        ...
                        if ((typeof val === 'undefined') || (val === null))
                            ...
                            if ((typeof con === 'undefined') || (con === null))
                                throw new Error('mongoose connection not open');
                            if (val.db === this.db.name && mongoose.models[val.namespace]) {
                                mongoose.models[val.namespace].findById(val.oid, callback);
                            } else {
...
exports.resolveDBRefs = resolveDBRefs;
